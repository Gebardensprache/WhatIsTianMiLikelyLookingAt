// 1.12.2 backport: the HWYLA compatibility shim. A STANDALONE build (`gradle -p shims/hwyla`):
// its settings includes the Jade root (`includeBuild("../../")`) so `snownee.jade:Jade` is
// substituted with the Jade project -- the shim compiles against Jade's output without any
// mavenLocal publication.
//
