...
/**
 * 1.12.2 backport: ported from The One Probe (mcjty/theoneprobe), minus the border rendering
 * (which used {@code RenderHelper.drawHorizontalLine} etc. through the real {@code mcjty.theoneprobe.rendering.RenderHelper})
 * -- the shim keeps the panel's layout math but does not paint a border since Jade's nested-box
 * already draws the box background.
 */
